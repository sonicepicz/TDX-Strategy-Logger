-- TDX Strategy Run Logger / Match Analyzer - Revision 4
-- Integrated revision of the supplied passive event-driven logger.
-- Not executed in Roblox here.
-- Audited against the supplied complete source extract. Runtime testing in Roblox is still required.
-- Both wrapper AttachCallback methods return nothing and retain no connection handles.
-- This observer connects directly to their exposed signals; only its own connections are disconnected.
--
...
local ResourceManager, NetworkingUtilities, TowerUtilities, Enums, EnemyBuffUtilities
...
local function descriptor(kind, data, isObject)
    ...
    if isObject then
        ...
        if kind == "towers" then
            ...
            d.skin, d.totalCost = data.SkinName, data.TotalCost
            d.position = data.SpawnCFrame and data.SpawnCFrame.Position
            d.spawnCFrame = data.SpawnCFrame
        end
    else
        d.hash = data[1]
        if kind == "towers" then
            ...
            d.skin, d.position, d.rotation, d.totalCost = data[3], data[4], data[5], data[24]
            d.initialAlive = data[15]
...
local function indexRecord(run, kind, r)
    ...
    local h = atom(r.hash)
    if h and not r.removed then run.byHash[kind][h] = r end
end
...
local function mergeDescriptor(run, kind, r, d)
    for _, key in ipairs({"hash", "uid", "spawn", "canonical", "display", "owner", "localOwner", "onePath", "skin", "position", "rotation", "spawnCFrame", "totalCost"}) do
...
local function buffData(r, data, previous)
    ...
    if config then
        ...
        if r.boss ~= nil then
            b.Amount = (r.boss or r.miniBoss) and config.BossPercentage or config.Percentage
        else
            b.Amount = nil
        end
...
local function buffEvent(run, r, action, data, w)
    ...
    local b = ((action == "REFRESHED" or action == "REMOVED") and previous) and copy(previous) or buffData(r, data, previous)
    local detail = copy(b)
...
local function captureEnemy(run, r, data, obj, source, w)
    ...
    if source == "CREATION" and type(data) == "table" then
        eventRecord(run, "enemies", r, "INITIAL REPLICATION", data, w)
    end
    if type(data) == "table" then
...
local function attackEvidence(r, obj)
    ...
    r.configuredCost = stats.TotalCost
    local result = {}
    local function add(data, path, availability)
        ...
        if data.DamageType ~= nil or data.Damage ~= nil or data.DamagePerSecond ~= nil then
            result[#result + 1] = {
                ...
                DPS = data == stats and r.configuredDPS or nil,
    ...
    if type(TowerUtilities.CalculateDPS) == "function" then
        local success, value = pcall(TowerUtilities.CalculateDPS, copy(stats))
        if success then r.configuredDPS = value end
    end
...
local function setWave(w, rewind)
    ...
    if active and active.status == "RECORDING" then
        ...
        active.waveTimeline = active.waveTimeline or {}
        active.waveTimeline[#active.waveTimeline + 1] = {wave = currentWave, from = old, FromRewind = rewind, time = utc()}
...
local function countCreation(run, kind, r, w, payloadOnly, token)
    if r.counted or r.classificationHandled then return end
    r.classificationHandled = true
    if kind == "towers" then
...
local function candidate(run, kind, data, token, w)
    ...
    task.defer(function()
        local ok, err = pcall(function()
            ...
            local obj = boundary == boundarySerial and getObject(kind, d.hash) or nil
            if r and r.removed then obj = nil end
...
local function towerUpgrades(batch)
    ...
    for _, data in pairs(batch) do
        if type(data) == "table" then
            ...
            if r then
                r.totalCost = data.TotalCost
                eventRecord(run, "towers", r, "UPGRADE REPLICATION", data)
...
local function enemyAnalysis(r)
    ...
    if EnemyBuffUtilities and type(EnemyBuffUtilities.GetStatMultipliers) == "function" then
        local ok, value = pcall(EnemyBuffUtilities.GetStatMultipliers, copy(r.buffs))
        lines[#lines + 1] = "Tracked-buff component multipliers (not final resistance): " .. (ok and serialize(value) or UNKNOWN)
    end
...
local function waveText(run, w)
    ...
    for _, id in ipairs(s.towers) do
        local r = towers[id]
        local label = clean(r.canonical or r.display)
...
local function summaryText(run)
    ...
    local lines = {
        ...
        "Mode: " .. mode,
        "PVP: " .. clean(m.IsPVP),
        "Wave timeline: " .. serialize(run.waveTimeline),
            ...
            lines[#lines + 1] = r.id .. " | Hash=" .. clean(r.hash)
                ...
                .. (kind == "enemies"
                    ...
                    or (" | Owner=" .. clean(r.owner) .. " | PlacementWave=" .. clean(r.placementWave)
                        ...
                        .. " | ObservedAt=" .. clean(r.observedAt) .. " | Position=" .. clean(r.position)
                        .. " | Rotation=" .. clean(r.rotation) .. " | SpawnCFrame=" .. clean(r.spawnCFrame)
                        .. " | Skin=" .. clean(r.skin) .. " | OwnedByLocalPlayer=" .. clean(r.localOwner)
                        .. " | TotalCost replicated=" .. clean(r.totalCost) .. " | Configured cost=" .. clean(r.configuredCost)
                        .. " | Configured DPS (not net damage)=" .. clean(r.configuredDPS)))
        end
    end
...
render = function()
    ...
    if not run then
        ...
        header.Text = "TDX STRATEGY LOGGER | " .. run.status
            ...
            .. " | Towers: " .. run.totals.towers .. " | Enemies: " .. run.totals.enemies .. " | Fake: " .. run.fakeExcluded
    ...
    newRunButton.Visible = coreReady
...
local function buildUI()
    ...
    connect(header.InputBegan, function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            dragInput, startPosition, startPointer = input, window.AbsolutePosition, input.Position
        end
    end)
    ...
    connect(UserInputService.InputChanged, function(input)
        ...
        window.Position = UDim2.fromOffset(
            math.clamp(startPosition.X + delta.X, 0, math.max(0, size.X - window.AbsoluteSize.X)),
            math.clamp(startPosition.Y + delta.Y, 0, math.max(0, size.Y - 60)))
    end)
end

-- Loading is bounded; never initialize, reset, spawn, mutate or call server methods.
local function loadModule(parent, name)
    local module = child(parent, name)
    if not module or not module:IsA("ModuleScript") then note("Module unavailable: " .. name); return nil end
    ...
    while alive and not done and os.clock() < deadline do task.wait(0.05) end
    if done and success and type(result) == "table" then return result end
    note("Module unavailable or timed out: " .. name)
    return nil
end
...
local function subscribe(name, callback, bindable)
    local ok, err = pcall(function()
        local signal
        if bindable then
            local wrapper = BindableHandler and BindableHandler.GetEvent(name)
            assert(wrapper and wrapper.BindableEvent, "Bindable unavailable")
            signal = wrapper.BindableEvent.Event
        else
            local remote = child(child(ReplicatedStorage, "Remotes"), name)
            assert(remote and remote:IsA("RemoteEvent"), "Remote unavailable")
            -- RemoteEventWrapperClass forwards this exact signal without decoding.
            -- Avoid GetEvent's unbounded wait and AttachCallback's nil return.
            signal = remote.OnClientEvent
        end
        connect(signal, function(...)
            if not alive then return end
            local success, message = pcall(callback, ...)
            if not success then note(name .. " observer failed: " .. clean(message)) end
        end)
    end)
    if not ok then note("Subscription unavailable: " .. name .. " | " .. clean(err)) end
    return ok
end

local function registerEvents()
    local towerOK = subscribe("TowerFactoryQueueUpdated", function(batch) factory("towers", batch) end)
    local enemyOK = subscribe("EnemyFactoryQueueUpdated", function(batch) factory("enemies", batch) end)
    local stateOK = subscribe("GameStateChanged", function(state, data)
        boundarySerial += 1
        stateChanged(state, data)
    end)
    subscribe("TowerUpgradeQueueUpdated", towerUpgrades)
    subscribe("TowerAliveStateChanged", towerAlive)
    subscribe("WaveChanged", setWave, true)
    subscribe("SpeedMultiplierChanged", function(v) updateMetadata({SpeedMultiplier = v}); requestRefresh() end)
    subscribe("AdditionalSpeedMultiplierChanged", function(v) updateMetadata({AdditionalSpeedMultiplier = v}); requestRefresh() end)
    subscribe("DifficultyChanged", function(v) updateMetadata({Difficulty = v}); requestRefresh() end)
    subscribe("GameMapChanged", function(v) updateMetadata(v); requestRefresh() end)
    subscribe("SetMapDifficulty", function(name, difficulty) updateMetadata({MapName = name, MapDifficulty = difficulty}) end, true)
    -- WaveChanged is also fired by GameClass.New, including reconstruction.
    -- Census here is enrichment/exclusion only, never a placement source.
    subscribe("TowerChanged", function(hash)
        if not active then return end
        local obj = getObject("towers", hash)
        local r = active.byHash.towers[atom(hash) or ""]
        if obj and r then updateTower(active, r, descriptor("towers", obj, true), obj, "LIVE CHANGE", currentWave) end
    end, true)

    local function pair(v) return v[1], v[2], v[3] end
    local function percentage(v)
        if NetworkingUtilities then return NetworkingUtilities.DecompressHashAndPercentage(v) end
        local h, n = decompress(v)
        return h, math.clamp(n * 9.536752259018191e-7, 0, 1)
    end
    local function number1(v)
        if NetworkingUtilities then return NetworkingUtilities.DecompressHashAndNumber1Place(v) end
        local h, n = decompress(v); return h, n / 10
    end
    local function field(name, key, decoder, transform)
        subscribe(name, function(batch)
            enemyBatch(name, batch, decoder, function(run, r, value)
                if transform then value = transform(value) end
                setState(run, r, key, value, name)
            end)
        end)
    end
    for _, item in ipairs({
        {"EnemiesSpeedChanged", "Speed", 0.0001},
        {"EnemiesDefaultSpeedChanged", "DefaultSpeed", 0.0001},
        {"EnemiesTopSpeedMultiplierChanged", "TopSpeedMultiplier", 0.00001},
        {"EnemiesBodyHeatPercentChanged", "BodyHeatPercentage", 0.01},
        {"EnemyAllDamageReductionChanged", "AllDamageReduction", 0.00001},
    }) do
        field(item[1], item[2], decompress, function(v) return v * item[3] end)
    end
    field("EnemyDamageResistanceModifierChanged", "DamageResistanceModifier", decompress, function(v) return (v - 100000) / 100000 end)
    for _, item in ipairs({
        {"EnemyMovementStateChanged", "Stopped"}, {"EnemiesStealthStatusChanged", "Stealth"},
        {"EnemyInvulnerableChanged", "Invulnerable"}, {"EnemyTakeNoDamageChanged", "TakeNoDamage"},
        {"EnemyAirUnitChanged", "IsAirUnit"},
    }) do field(item[1], item[2], decompress, function(v) return v == 1 end) end
    for _, item in ipairs({
        {"EnemiesMaxHealthChanged", "MaxHealth"}, {"EnemiesMaxShieldChanged", "MaxShield"},
        {"EnemyMaximumDamageChanged", "MaximumDamage"}, {"EnemyAnimationStateChanged", "AnimationState"},
        {"EnemyBountyChanged", "BountyCount"}, {"EnemyStasisChanged", "UnstasisStarted"},
    }) do field(item[1], item[2], pair) end
    field("EnemiesHealthChanged2", "Health", decompress)
    field("EnemiesShieldChanged2", "Shield", decompress)
    for _, item in ipairs({{"EnemiesHealthChanged1", "Health", "MaxHealth"}, {"EnemiesShieldChanged1", "Shield", "MaxShield"}}) do
        subscribe(item[1], function(batch)
            enemyBatch(item[1], batch, percentage, function(run, r, value)
                setState(run, r, item[2] .. "Percentage", value, item[1])
                local maximum = r.state[item[3]]
                setState(run, r, item[2], type(maximum) == "number" and math.ceil(maximum * value) or UNKNOWN, item[1])
            end)
        end)
    end
    subscribe("EnemiesStunnedStatusChanged", function(batch)
        enemyBatch("EnemiesStunnedStatusChanged", batch, pair, function(run, r, value)
            setState(run, r, "ActiveStuns", value, "REPLICATION")
            setState(run, r, "Stunned", type(value) == "table" and #value > 0, "REPLICATION")
        end)
    end)
    subscribe("EnemiesExtraDamageChanged", function(batch)
        enemyBatch("EnemiesExtraDamageChanged", batch, pair, function(run, r, value, extra)
            setState(run, r, "DamageMultiplier", value, "REPLICATION")
            setState(run, r, "ExtraDamageMultiplier", extra, "REPLICATION")
        end)
    end)
    for _, item in ipairs({{"EnemyApplyBuffData", "APPLIED"}, {"EnemyRemoveBuffData", "REMOVED"}, {"EnemyRefreshBuffData", "REFRESHED"}}) do
        subscribe(item[1], function(batch)
            enemyBatch(item[1], batch, function(v) return v.EnemyHash, v end,
                function(run, r, value) buffEvent(run, r, item[2], value) end)
        end)
    end
    subscribe("EnemiesSecondLifeActivated", function(batch)
        enemyBatch("EnemiesSecondLifeActivated", batch, function(v) return v, true end,
            function(run, r, value)
                setState(run, r, "OnSecondLife", value, "SECOND LIFE EVENT - NOT CREATION")
            end)
    end)
    subscribe("EnemiesBurningUpdate", function(batch)
        for i = 1, #batch, 2 do
            local h, duration = number1(batch[i])
            if active then
                local r = enemyForEvent(active, h)
                eventRecord(active, "enemies", r, "BURN UPDATE", {Hash = h, BurnTimeLeft = duration, BurnType = batch[i + 1]})
            end
        end
    end)
    for _, name in ipairs({"EnemyFaceGoalUntilMove", "EnemyTeleported", "EnemyKnockedBack"}) do
        subscribe(name, function(batch)
            enemyBatch(name, batch, pair, function(run, r, _, _, payload) eventRecord(run, "enemies", r, name, payload) end)
        end)
    end
    subscribe("EnemyUsedAbility", function(payload)
        if active then eventRecord(active, "enemies", enemyForEvent(active, payload[1]), "ABILITY USED", {AbilityData = payload[2]}) end
    end)
    local function towerEvent(name, hash, detail)
        local run = active
        if not run or run.status ~= "RECORDING" then return end
        local r = run.byHash.towers[atom(hash) or ""]
        eventRecord(run, "towers", r, name, detail)
    end
    for _, name in ipairs({"TowerTargetChanged", "TowerConvertedTargetChanged", "TowerHealTargetChanged", "TowerAttack", "TowerConvertedAttack", "TowerAttackDisabledChanged"}) do
        subscribe(name, function(batch)
            for _, v in ipairs(batch) do
                towerEvent(name, v.X, {TowerHash = v.X, TargetOrValue = v.Y, NetDamage = UNKNOWN})
            end
        end)
    end
    for _, name in ipairs({"TowerChainAttack", "TowerConvertedChainAttack", "TowerAttackPosition", "TowerAbilityUsed", "TowerAbilityCancelled", "TowerMainAttackOverrideChanged"}) do
        subscribe(name, function(batch)
            for _, v in ipairs(batch) do towerEvent(name, v[1], v) end
        end)
    end
    subscribe("ShipTurretFired", function(h, turret, barrel, target, converted)
        towerEvent("ShipTurretFired", h, {Turret = turret, Barrel = barrel, TargetHash = target, TargetIsTower = converted, NetDamage = UNKNOWN})
    end)
    for i = 1, 5 do
        for _, prefix in ipairs({"EnemyTargetChanged", "EnemyAttack"}) do
            local name = prefix .. i
            subscribe(name, function(batch)
                enemyBatch(name, batch, pair, function(run, r, target, pathEntity, payload)
                    eventRecord(run, "enemies", r, name, {Index = i, TargetHash = target, TargetIsPathEntity = pathEntity, TargetIsMapEntity = payload[4]})
                end)
            end)
        end
    end
    return towerOK and enemyOK and stateOK
end
...
local ok, message = pcall(function()
    ...
    if Enums then
        states, difficulties = Enums.GameStates or {}, Enums.Difficulties or {}
        damageTypes, buffTypes = Enums.DamageTypes or {}, Enums.EnemyBuffTypes or {}
    end
    ResourceManager = loadModule(common, "ResourceManager")
    TowerUtilities = loadModule(common, "TowerUtilities")
    NetworkingUtilities = loadModule(common, "NetworkingUtilities")
    EnemyBuffUtilities = loadModule(common, "EnemyBuffUtilities")
    ...
    if not alive then return end
    local obj = currentGame()
    if obj then updateMetadata(obj); currentWave, gameState = waveKey(obj.WaveNumber), obj.State end
    beginRun("Client-visible history starts at observer attachment; initial/reconstruction entities are not counted as new creations", nil, true)
    coreReady = registerEvents() and GameClass ~= nil and TowerClass ~= nil and EnemyClass ~= nil
    note("No match identifier is exposed by these sources. Rewinds retain identities; use NEW RUN for a new observation.")
    note("Raw creation payloads are the historical evidence; live lookup is enrichment only. Cross-event delivery order and gaps before attachment are not recoverable.")
    note("Both AttachCallback wrappers return nil. Logger uses direct exposed signals and disconnects only its own handles on reinjection.")
    note("Enemy HealthHandler constructor argument names and MovementHandler initial speed layout are not included in this extract; initial raw payloads retained, numeric health/speed established by subsequent events.")
    if gameState == states.EndScreen then finishRun(nil, false) end
    requestRefresh()
end)
if not ok then
    coreReady = false
    note("Initialization failed: " .. clean(message))
    warn("TDX Logger initialization: " .. clean(message))
    requestRefresh()
end
-- End of complete logger.
